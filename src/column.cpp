...
void Column::setImportance(int i_importance) {
    if (i_importance < -128) {
        m_importance = -128;
        if (m_verbose) {std::cerr << "Warning: Attempted to set 'importance' to value lower than -128, clamping to -128\n";}
        return;
    }
    if (i_importance > 127) {
        m_importance = 127;
        if (m_verbose) {std::cerr << "Warning: Attempted to set 'importance' to value higher than 127, clamping to 127\n";}
        return;
    }
